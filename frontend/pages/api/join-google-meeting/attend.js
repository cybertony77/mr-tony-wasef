  ...
  try {
    ...
    if (sessionDelta !== 0) {
      await recordPaymentSessionChange(db, parseInt(studentId), {
        delta: sessionDelta,
        type: 'google_meeting',
        reason: `Live Google Meet attendance for ${lesson}`,
        lesson,
        by: 'student',
      });
    }
    ...
    try {
      ...
      console.error('⚠️ Failed to add Google Meet attendance to history:', historyErr);
    }
      ...
      try {
        ...
        if (!existingScoringHistory) {
          ...
          try {
            ...
            if (calcRes.ok) {
              console.log(`[SCORING] Attend score applied for student ${studentId}, lesson "${lesson}" via Google Meet`);
            } else {
        ...
        console.error('⚠️ Failed to process scoring for Google Meet attendance:', scoringErr);
      }
    }
    ...
    console.error('Error in Google Meet meeting attend API:', error);
