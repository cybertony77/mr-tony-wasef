...
export default function VerificationVideoCodes() {
  ...
  const canManageVvc = canAccessVvcVhc(systemConfig, profile?.role);
  ...
  useEffect(() => {
    ...
    if (!profileLoading && profile && systemConfig && !canManageVvc) {
      router.push('/dashboard/manage_online_system');
    }
  }, [systemConfig, isOnlineVideosEnabled, router, profileLoading, profile, canManageVvc]);
  
  // Don't render if feature is disabled or role not allowed
  if ((systemConfig && !isOnlineVideosEnabled) || (!profileLoading && profile && systemConfig && !canManageVvc)) {
    return null;
  }
  const pageSize = 100;
